 ...
 *
 * 검증 실패 영역 (필수 필드 누락 / id 불일치 / JSON 형식 오류) 도 entry 반환 —
 * dashboard 안 빨간 알림 영역 시각.
 *
 * stale 영역: measuredAt 가 7일 이상 지난 plugin 자료 — 회색 배지 영역 시각.
 */
 ...
 * + id 일치 영역. 외부 plugin 개발자 측 가이드 (docs/plugin-development.md Section 3.2)
 * 와 정합.
 */
function validatePluginJson(
  ...
  if (typeof o.id !== "string") {
    return { ok: false, reason: "id 필드 빠짐 또는 string 빠짐" };
  }
  if (o.id !== expectedId) {
    return {
      ok: false,
      reason: `id 영역 (${o.id}) ≠ 폴더 이름 (${expectedId})`,
    };
  }
  if (typeof o.label !== "string") {
    return { ok: false, reason: "label 필드 빠짐 또는 string 빠짐" };
  }
  if (typeof o.measuredAt !== "string") {
    return { ok: false, reason: "measuredAt 필드 빠짐 또는 string 빠짐" };
  }
  if (!o.summary || typeof o.summary !== "object") {
    return { ok: false, reason: "summary 필드 빠짐 또는 객체 빠짐" };
  }
  const summary = o.summary as Record<string, unknown>;
  if (!summary.primary || typeof summary.primary !== "object") {
    return { ok: false, reason: "summary.primary 영역 빠짐 또는 객체 빠짐" };
  }
  const primary = summary.primary as Record<string, unknown>;
  if (typeof primary.label !== "string") {
    return { ok: false, reason: "summary.primary.label 영역 빠짐" };
  }
...
/**
 * measuredAt 영역이 STALE_THRESHOLD_DAYS (7일) 이상 지난 시점 영역 검증.
 * 자연 잘못된 measuredAt 영역 (parse 실패) 은 stale=false (사용자 인지 빠짐 회피).
 */
