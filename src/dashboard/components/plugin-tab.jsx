...
// 본 컴포넌트는 단일 plugin entry 영역 받음 — root.jsx 가 탭별로 분기.
...
function PluginTab({ entry }) {
  ...
  // measuredAt 영역 — ISO 8601 → "YYYY-MM-DD HH:mm" 형식 (분 단위)
  const measuredAtFmt = (output.measuredAt || "").slice(0, 16).replace("T", " ");

  // details 표 영역 — 모든 행의 키 합집합 영역 (name 빠짐, 알파벳 순)
  ...
  return (
    <div>
      {/* 정정 3: 별도 헤더 (lh-sect-head) → 본문 시작 인라인 영역 (Figma 패턴 정합). */}
      <div className="tab-stamp">
        ...
        {stale && (
          <>
            ...
            <span className="tag stale"><span className="tdot" />자료 7일+ 오래됨</span>
          </>
        )}
      </div>
      ...
      {output.details && output.details.length > 0 ? (
        ...
        <div className="skeleton-note">
          <span className="tdot" />
          details 영역 빠짐 — Summary 탭의 Layer 영역에서 카드 영역 확인.
        </div>
      )}
    </div>
  );
    ...
    <div>
      ...
      <section className="card below-threshold">
        <div className="card-head">
          <div className="card-label">plugin 자료 영역 검증 실패</div>
        </div>
...
// === root.jsx 동기화 영역 (시안 lighthouse-tab.jsx / figma-tab.jsx 같은 패턴) ===
